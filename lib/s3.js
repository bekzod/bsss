...
module.exports = exports = function (options){
    ...
    S3.singleUpload = function (path,buffer,bufferHash,callback) {
        return self.awsRequest('PUT',path,buffer,bufferHash,function(err,res,json){
    ...
    S3.multipartInitiateUpload = function(path,callback) {
        path+='?uploads';
        return self.awsRequest('POST',path,null,null,function(err,res,json){
            if (err) return callback(err);
    ...
    S3.multipartAbortUpload = function (path,uploadID,callback) {
        ...
        return self.awsRequest('DELETE',path,body,bodyHash,function(err,res,json){
            if (err)return callback(err);
    ...
    S3.multipartCompleteUpload = function (path,uploadID,partsRef,callback) {
        ...
        return self.awsRequest('POST',path,body,null,function(err,res,json){
            if (err)return callback(err);
