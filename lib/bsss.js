...
Bsss.prototype.abort = function(){
    ...
    for (var i = self.partsUploaded; i >= 0; i--) {
        if(self.requests[i])self.requests[i].abort();
    };
...
Bsss.prototype.chunkFinished = function(err,opts,self){
    ...
    if(err){
        ...
        if (self._isWriteFinished && self.finishedUploads.length == self.partsUploaded){
            ...
            if(self.uploadsQue.active.length < self.concurrency)self.emit('drained');
        };
    };  
};
...
Bsss.prototype.write = function(chunk){
    var self = this;
    if(chunk && self.uploadsQue.active.length < self.concurrency){
