...
Bsss.prototype._uploadBuffer= function(){
    ...
    opts.retries = 0;
    console.log(self.partsUploaded);
...
Bsss.prototype.chunkFinished = function(err,opts,self){
    ...
    if(err){
        ...
        if(self._isWriteFinished && self.isUploadFinished){
            self._finish();
        }else{          
            if(!self.uploadsQue.length())self.emit('drained');
        };
...
Bsss.prototype.isUploadFinished = function(){
    var self = this;
...
Bsss.prototype.finish = function(){
    ...
    else if(self.isUploadFinished()) self._finish();
};
