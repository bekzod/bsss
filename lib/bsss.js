...
Bsss.prototype._uploadBuffer= function(){
    ...
    self.buffer       = null;
    self.buffer       = new Buffer(self.bufferSize,'utf8');
    self.dataInBuffer = 0;
...
Bsss.prototype.chunkFinished = function(err,opts,self){
    ...
    if(err){
        ...
        if(self._isWriteFinished && self.hasUploadsFinished()){
...
Bsss.prototype._write = function(chunk){
    ...
    if(amountToBeFull < chunk.length){
        self.buffer.write(chunk,self.dataInBuffer,amountToBeFull);
        self.dataInBuffer += amountToBeFull;
        return chunk.slice(amountToBeFull);
    }else{
        self.buffer.write(chunk,self.dataInBuffer,chunk.length);
...
Bsss.prototype.hasUploadsFinished = function(){
    return this.finishedUploads.length == this.partsUploaded;
};
...
Bsss.prototype._finishMultiUpload = function(){
    var self = this;
    self.storage.multipartCompleteUpload(
     self.path,
     self.uploadId,
     self.finishedUploads,
     function(err,res){
...
Bsss.prototype.finish = function(){
    ...
    if(self.partsUploaded > 0){
        if(self.dataInBuffer){
         self._uploadBuffer();
        }else if(self.hasUploadsFinished()){
            self._finishMultiUpload();
        }
    }else{
