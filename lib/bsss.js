...
Bsss.prototype.abort = function(){
    ...
    for(var req in self.requests){
        if(req.hasOwnProperty('abort'))req.abort();
    }
...
Bsss.prototype.chunkFinished = function(err,opts,self){
    ...
    self.requests || (self.requests[opts.partId] = null)
    ...
    if(err){
        ...
        else if(self.hasSpaceInQue())self.emit('drain');
    };  
};


Bsss.prototype.hasSpaceInQue= function(){
    return this.uploadsQue.active.length < this.concurrency;
};


Bsss.prototype.hasUploadsFinished= function(){
    return this._isWriteFinished && this.finishedUploads.length == this.partsUploaded;
};
